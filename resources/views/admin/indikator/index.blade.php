          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ route('admin.index') }}">Home</a></li>
               <li class="breadcrumb-item"><a href="{{ route('masterperingkat.index') }}">Master Peringkat</a></li>
        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
              ...
              <!-- /.card-header -->
              <div class="card-body">
                <table id="tbl-indikator" class="table table-bordered table-striped">
                 <thead>
                  <tr>
                    <th>No</th>
                    <th>Indikator</th>
...
$(document).ready(function() {
    ...
    $('#tbl-indikator').DataTable({
        processing: true,
        ordering: true,
        serverSide: false,
        ajax: {
            url: "{{ route('admin.indikator.data', ':id') }}".replace(':id', id),
        },
        columns: [
            {
                data: null,
                render: (data, type, row, meta) => `<div class='text-center'>${meta.row + 1}.</div>`
            },
            {
                data: 'name',
  ...
  // tambah data
$(document).on('submit','#addIndikator', function(e){
  let url = window.location.pathname;
  let id = url.split('/').pop();
  $('#rank_id').val(id);
...
$('.table').on('click','.edit-btn', function(){
  var id = $(this).data('id');
  showLoading();
  var url = "{{ route('indikator.edit',':id') }}".replace(':id',id);
  $.get(url, function(data){
    closeLoading();
    $('#editID').val(data.id);
    $('#indikator').val(data.name);
  var updateUrl ="{{ route('indikator.update',':id') }}".replace(':id',data.id);
...
$('.table').on('click','.delete-btn', function(){
  var table = $('.table').DataTable();
  var id    = $(this).data('id');
  var url   = "{{ route('indikator.destroy',':id') }}".replace(':id',id);
