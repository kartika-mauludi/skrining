    ...
    $(document).ready(() => {
            ...
            reader.onload = function (event) {
                ...
                if (sheet.length > 1) {
                    sheet.slice(1).forEach((row, index) => {
                        let feedback_deskripsi       = row[0] != null ? String(row[0]).trim() : "";
                        let status                  = row[1] != null ? String(row[1]).trim() : "";
                         if ( !feedback_deskripsi && !status) {
                              return;
                            }
                        let errorMessage = "";

                        if (!feedback_deskripsi) errorMessage += "feedback_deskripsi kosong ";
                        if (!status) errorMessage += "status kosong ";
        ...
        $("#btnUploadData").click(function () {
            ...
            $("#importPreviewTable tbody tr").each(function () {
                ...
                let feedback_deskripsi  = row.eq(1).text().trim();
                let status              = row.eq(2).text().trim();
                ...
                tableData.push({ feedback_deskripsi, status });
            });
            ...
            fetch(`{{ route(('admin.tanggapan.import')) }}`, {
                method: "POST",
                headers: {
                ...
                let table = $('#tbl-tanggapan').DataTable()
                function read() {
                    reader.read().then(({ done, value }) => {
                        if (done) {
                            ...
                            table.clear().draw();
                            table.ajax.url("{{ route('admin.tanggapan.data') }}").load();
                            return;
                        }
